...
st.markdown("---")
# Uploads por capítulo (agora com 7)
capitulos = {}
for i in range(7):
...
def processar_pdfs(pdf_mae_file, capitulos, pos_x, pos_y):
   ...
   if total_paginas < 7:
       raise Exception("O PDF PRINCIPAL precisa ter pelo menos 7 páginas para conter as capas dos capítulos.")
   # Dividir PDF Mãe em corpo e capas (agora 7 capas)
   corpo_principal = reader.pages[:-7]
   capas = reader.pages[-7:]
   paginas_finais = []
   ...
   for i in range(7):
       ...
       for file in arquivos_anexos:
           ...
           for pagina_anexo in leitor_anexo.pages:
               ...
               # Não adiciona os anexos à numeração

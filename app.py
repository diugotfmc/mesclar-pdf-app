...
import io
st.set_page_config(page_title="Mesclar Relatório com Anexos", layout="wide")
st.title("📘 Mesclar Relatório com Anexos por Capítulo")
# ===============================
...
capitulos = {}
for i in range(6):
   st.markdown(f"### 📎 Capítulo {i+1}")
   files = st.file_uploader(f"Anexos para o Capítulo {i+1}", type=["pdf"], accept_multiple_files=True, key=f"cap_{i+1}")
...
def processar_pdfs(pdf_mae_file, capitulos, pos_x, pos_y):
   ...
   if total_paginas < 6:
       raise Exception("O PDF Mãe precisa ter pelo menos 6 páginas para conter as capas dos capítulos.")
